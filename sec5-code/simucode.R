...
# assume data from 3 clusters: related to 3 regions
# for each region, set a target enrollment 
# remove data when the enrollment + 15 finished  
...
# number of centers
J <- 40
# number of centers in each cluster
#nc <- rep(0, J)
#while(sum(nc == 0) > 0){
#  nc <- rmultinom(n = 1, size = J, prob = c(0.2, 0.3, 0.5))  #c(15, 15, 20)
#}
nc <- c(15, 15, 10)
# number of patients in each center
prob <- sample(c(2, 2, 3, 4), J, replace = TRUE)
prob <- prob/sum(prob)
n <- rep(0, J * 20)
while(sum(n == 0) > 0){
  n <- rmultinom(n = 1, size = J * 50, prob = prob)
}



#c(15, 15, 15, 15, 15, 10, 10, 10, 10, 10, 
...
thetak <- c(0.5, 1, 6)
rk <- matrix(c(2, 1, 0.5, 2, 4, 1, 1, 1, 0.5), byrow = TRUE, nrow = 3)
#thetak <- c(1, 1, 1)
#rk <- matrix(c(1, 1, 1, 1, 1, 1, 1, 1, 1), byrow = TRUE, nrow = 3)
#=========================================================
...
data1$exclude <- as.numeric(data1$time > quantile(data1$time, 0.7))
...
#=================================================================
# get center gap before close time
...
#==================================================================
# adjust closing time and data according to pre specified target enrollment
...
center.list2 <- c(11:15)[11:15 %in% center.info$center]
center.list3 <- c(16:30)[16:30 %in% center.info$center]
center.list4 <- c(31:40)[31:40 %in% center.info$center]
...
if(nrow(finish.time1.type1) > 0 & nrow(finish.time1.type2) > 0){
  finish.time1 <- max(finish.time1.type1$enroll.time, finish.time1.type2$enroll.time)+7
}else{
...
if(nrow(finish.time2.type1) > 0 & nrow(finish.time2.type2) > 0){
  finish.time2 <- max(finish.time2.type1$enroll.time, finish.time2.type2$enroll.time)+7
}else{
  print('sample number insufficient')
  break
}
...
if(nrow(finish.time3.type1) > 0 & nrow(finish.time3.type2) > 0){
  finish.time3 <- max(finish.time3.type1$enroll.time, finish.time3.type2$enroll.time)+7
}else{
  print('sample number insufficient')
  break
}
#finish.time4.type1 <- data1 %>% filter(center %in% center.list4 & status == 1) %>% 
#  arrange(enroll.time) %>% slice(target4.1:target4.1) %>% 
#  dplyr::select(enroll.time) + 7
#finish.time4.type2 <- data1 %>% filter(center %in% center.list4 & status == 2) %>% 
#  arrange(enroll.time) %>% slice(target4.2:target4.2) %>% 
#  dplyr::select(enroll.time) + 7
#finish.time4 <- max(finish.time4.type1$enroll.time, finish.time4.type2$enroll.time)

#finish.time2 <- data1 %>% filter(center %in% center2 & status %in%c(1, 2)) %>% 
#  arrange(enroll.time) %>% slice(target2:target2) %>% 
#  dplyr::select(enroll.time) + 14
#finish.time3 <- data1 %>% filter(center %in% center3 & status %in%c(1, 2)) %>% 
#  arrange(enroll.time) %>% slice(target3:target3) %>% 
#  dplyr::select(enroll.time) + 14
...
if(nrow(screen.time.wait1) > 0){
  ...
  #screen.time.wait$time.wait[screen.time.wait$center %in% screen.time.wait1$center] <- screen.time.wait1$gap.time.close
}
...
#===========================================================================
# waiting time forparametric model
#===================================================================
# add waiting time information to the training data
data1.nofail <- data1 %>% filter(status %in% c(0, 1, 2))
new.iat <- data1.nofail %>% group_by(center) %>% 
  mutate(iat.new = c(head(screen.time - open.time, n = 1), diff(screen.time))) 
data1.nofail$iat <- new.iat$iat.new
data1.train.nofail <- data1.nofail %>% filter(screened == 1)
data1.train.nofail$censor <- 1

# calculate waiting time for screening process
# center: already start screening 
screen.time.wait.nofail <- data1.nofail %>% filter(screened == 1 ) %>% 
  group_by(center) %>% 
  summarise(time.wait = cutoff - max(screen.time), 
            period = 0)
# adjust waiting time for closed center
screen.time.wait1.nofail <- center.info %>% filter(close == 1) %>% 
  dplyr::select(center, gap.time.close)
if(nrow(screen.time.wait1.nofail) > 0){
  screen.time.wait.nofail <- screen.time.wait.nofail %>% 
    left_join(screen.time.wait1.nofail, by = c('center'))
  adj.id <- which(!is.na(screen.time.wait.nofail$gap.time.close))
  screen.time.wait.nofail$time.wait[adj.id] <- screen.time.wait.nofail$gap.time.close[adj.id]
  screen.time.wait.nofail <- screen.time.wait.nofail %>% dplyr::select(-gap.time.close)
}
# opened not yet screened center
center.id <- center.info$center[center.info$open == 1 & center.info$start == 0]
if(length(center.id) > 0){
  screen.time.wait2.nofail <- data1.nofail[!duplicated(data1.nofail$center), ] %>% 
    filter(center %in% center.id) %>% 
    mutate(time.wait = cutoff - open.time, period = 1) %>%
    dplyr::select(center, time.wait, period)
  screen.time.wait.nofail <- rbind(screen.time.wait.nofail, screen.time.wait2.nofail)
}
screen.time.wait.nofail <- screen.time.wait.nofail %>% left_join(center.info, by = c('center'))
screen.time.wait.nofail <- data.frame(screened = 0,
                                      center = screen.time.wait.nofail$center, 
                                      iat = screen.time.wait.nofail$time.wait,
                                      gap.time.open = screen.time.wait.nofail$gap.time.open, 
                                      gap.time.enroll = 0, 
                                      plan.time = screen.time.wait.nofail$plan.time, 
                                      open.time = screen.time.wait.nofail$open.time,
                                      screen.time = 0, 
                                      enroll.time = 0,
                                      status = 0, 
                                      censor = 0, 
                                      enroll = 0, 
                                      period = screen.time.wait.nofail$period)
