...
data.new.pre.adj <- lapply(data.new.pre, FUN = function(x){
  ...
  #finish.time4.type1 <- x %>% filter(center %in% center.list4 & status == 1) %>% 
  #  arrange(enroll.time) %>% slice(target4.1:target4.1) %>% 
  #  dplyr::select(enroll.time)
  #finish.time4.type2 <- x %>% filter(center %in% center.list4 & status == 2) %>% 
  #  arrange(enroll.time) %>% slice(target4.2:target4.2) %>% 
  #  dplyr::select(enroll.time)
  #finish.time4 <- max(finish.time4.type1$enroll.time, finish.time4.type2$enroll.time)+7
  ...
  #x <- x %>% filter(!(center %in% center.list4 & screen.time >= finish.time4)) 
  ...
  #x$screen.time <- x$screen.time/30
  #x$enroll.time <- x$enroll.time/30
  return(x)
})
...
time <- lapply(data.new.pre.adj, function(x){
  t1 <- max(x %>% filter(center %in% center.list1) %>% dplyr::select(enroll.time))
  t2 <- max(x %>% filter(center %in% center.list2) %>% dplyr::select(enroll.time))
  t3 <- max(x %>% filter(center %in% center.list3) %>% dplyr::select(enroll.time))
  return(c(t1, t2, t3))
})
t1 <- sapply(time, function(x) x[1])
c(median(t1), quantile(t1, c(0.025,0.975)))
t2 <- sapply(time, function(x) x[2])
c(median(t2), quantile(t2, c(0.025,0.975)))
t3 <- sapply(time, function(x) x[3])
c(median(t3), quantile(t3, c(0.025,0.975)))
## make plot:

### basic codes for screening prediction only

# for(i in 1:n.repeat){
#   centers.new <- centers
#   new <- matrix(0, nrow = h*J.pre, ncol = 2)
#   new[, 2] <- 1
#   sampleid <- sample(n.burnin:niter, 1)
#   lambda <- iter.data$lambda[sampleid]
#   if(sum(centers$type == 3) > 0){
#     centers.new$basetime[centers$type == 3] <- 
#       apply(center3, 1, FUN = function(x){x[2] + runif(1, x[3], max.time.open)})
#   }
#   for(j in seq(J.pre)){
#     if(centers$type[j] == 1){ # opened, screened center
#       j.id <- which(center.train == centers$center[j])
#       uj<- iter.data$u[sampleid, j.id]
#       trunc <- c(centers$wait.time[j], rep(0, h - 1))
#     }else if(centers$type[j] == 2){ # opened, not screened yet center
#       theta.sampled <- iter.data$theta[sampleid, ]
#       thetaj <- sample(theta.sampled, size = 1)
#       uj <- rgamma(1, shape = thetaj, rate = thetaj) + 0.0001
#       trunc <- c(centers$wait.time[j], rep(0, h - 1)) 
#     }else{ # planned, not opened yet center
#       theta.sampled <- iter.data$theta[sampleid, ]
#       thetaj <- sample(theta.sampled, size = 1)
#       uj <- rgamma(1, shape = thetaj, rate = thetaj) + 0.0001
#       trunc <- rep(0, h) 
...
#   }
#   new <- as.data.frame(new)
#   colnames(new) <- c('iat.new', 'screened')
#   new$center <- rep(centers$center, each = h)
#   time.exact.new <- new %>% group_by(center) %>% summarise(time = cumsum(iat.new))
#   new$time.exact <- time.exact.new$time
#   new <- new %>% left_join(centers.new, by = c('center'))
#   new$time.exact <- new$time.exact + new$basetime
#   new$type <- 'predict'
#   new <- new %>% arrange(time.exact) #%>% mutate(cum.hire = cumsum(screened))
#   #new$cum.hire <- new$cum.hire + sum(plotmat$type == 'train')
#   #new <- new[new$center %in% include, ]
#   new <- new %>% dplyr::select(screened, time.exact, type)
#   data.new[[i]] <- new
# }
# 
# 
# 
# ## make plot:
# plotdata <- data1.train %>% dplyr::select(screened, time.exact) %>% mutate(type = 'real')
# for (k in 1:n.repeat) {
#   data.new[[k]] <- data.new[[k]] %>% filter(time.exact < cutoff + 1000)
#   data.new[[k]] <- rbind(plotdata, data.new[[k]])
# }
# 
# date_min <- min(sapply(data.new, FUN = function(x) {
#   min(x$time.exact, na.rm = TRUE)}))
# date_max <- max(sapply(data.new, FUN = function(x) {
#   max(x$time.exact, na.rm = TRUE)}))
# dates <- seq(date_min, date_max, by = 1)
# dates <- unique(c(dates, date_max))
# # NUMBER OF PATIENTS OVER TIME - PREDICTION -------------------------------
# 
# # NUMBER OF ARRIVALS WITHIN EACH INTERVAL
# counts <- sapply(data.new, FUN = function(x)
#   hist(x      = x$time.exact,
#        plot   = FALSE,
#        breaks = dates,
#        right  = FALSE)$counts
...
#   geom_vline(xintercept = as.numeric(cutoff), colour = "darkgray",
#              size = 0.7, linetype = "longdash") + 
#   geom_line(data = plotmat2, aes(x = time.exact, y = cum.hire), color = 'red'
#             , size = 1) + 
#   theme(legend.position="none") +
#   xlim(c(0, 950)) + ylim(c(0, 1500))
# 
# # plot each individual line
# i <- 6
# plotmat3 <- data.new[[1]] %>% filter(type == 'predict')
# plotmat3$cum.hire <- cumsum(plotmat3$screened) + max(plotmat$cum.hire)
# plotmat3 <- rbind(plotmat, plotmat2, plotmat3)
# ggplot(plotmat3, aes(time.exact, cum.hire, col = type)) + 
#   geom_line() + xlim(c(0, 950))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     
# 
